...
import { extractTableOfContents } from '../utils/pdfExtractor';
...
export function ChapterList() {
  ...
  const handleExtractToc = async (pdfBytes: ArrayBuffer) => {
    try {
      setIsProcessing(true);
      setError(null);
      
      const tocEntries = await extractTableOfContents(pdfBytes);
      
      if (tocEntries.length === 0) {
        throw new Error('Tidak ada daftar isi yang ditemukan dalam PDF');
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2">
        ...
        <button
          onClick={() => {
            const pdfBytes = window.localStorage.getItem('lastGeneratedPdf');
            if (pdfBytes) {
              handleExtractToc(new Uint8Array(pdfBytes.split(',').map(Number)).buffer);
            } else {
              setError('Tidak ada PDF yang tersedia. Silakan generate PDF terlebih dahulu.');
            }
          }}
          className={buttonClass}
        ...
        >
          <Plus className="w-4 h-4 mr-2" />
          {isProcessing ? 'Memproses...' : 'Tambah Daftar Isi'}
        </button>
